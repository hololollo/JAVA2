...
// Pattern : 특정 데이터가 "형식"이 맞는지 비교하려고 할 경우 특정 데이터 형식을 지정하는 클래스
// Matcher : 특정 패턴의 형태가 맞는지 비교하는 클래스
// 정규식(Reg Expression) => regEx
public class PatternExam1 {
	public static void main(String[] args) {
		String[] dt = {"bag","kim1004","9to6","8to5","single","Mom","ABC","강범준","^_^"};
		Pattern p = Pattern.compile("[a-z]*"); // 영문 소문자로만 구성된 데이터  (* : 전체)
		...
		// Pattern p = Pattern.compile("e$"); // 특정문자로 끝나는 경우 문자 뒤에 $(Web)
		// Pattern p = Pattern.compile("^b"); // 특정문자로 시작하는 경우 문자 앞에 ^(Web)
